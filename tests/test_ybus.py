...
class TestTransformer:
    ...
    def test_dc_b_prime_matches_ybus_transfer(self) -> None:
        # B' DC-расчёта = мнимая часть взаимной проводимости Ybus: |t|/x.
        from gridpf.solvers.dc_pf import build_b_prime

        for tap in (0.9, 1.0, 1.196):
            net = _toy_two_bus(x=0.5, tap=tap)
            ybus = build_ybus(net)[0].toarray()
            b = build_b_prime(2, net.from_idx, net.to_idx, net.branch_x, net.tap_ratio).toarray()
            np.testing.assert_allclose(-b[0, 1], ybus[0, 1].imag, rtol=1e-12)
